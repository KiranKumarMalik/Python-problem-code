...
for i in range(n): # i is the row number upto n (in range if n=5 then i=0,1,2,3,4 because range is esclusive)
    for space in range((n-i)):  # space is the number of space upto n-i (when i=0 then space=n-i=5), it will print upto 4 spaces {Logic: Total number of space= total number of row - row number}
